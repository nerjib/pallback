...
router.get('/allpu/:lga', async (req, res) => {
  ...
  router.get('/getalllgpu/:type/:lga', async (req, res) => {
    const getAllQ = `SELECT * FROM kdpunits where type=$1 and lga=$2  order by lga asc, ward asc, puid asc`;
    try {
      // const { rows } = qr.query(getAllQ);
  ...
  router.get('/getward/:lga', async (req, res) => {
    const getAllQ = `SELECT distinct ward FROM kdpunits where lga=$1  order by ward asc`;
    try {
      // const { rows } = qr.query(getAllQ);
  ...
  router.get('/allpuids', async (req, res) => {
    const getAllQ = `SELECT distinct pword, uname,ward,lga,puid,puname,remark FROM kdpunits order by lga asc, ward asc, puid asc`;
    try {
      // const { rows } = qr.query(getAllQ);
      const { rows } = await db.query(getAllQ);
      return res.status(201).send(rows);
  ...
  router.get('/lga/:lga/:type', async (req, res) => {
    const getAllQ = `SELECT sum(apc) as apc, sum(pdp) as pdp, sum(others) as others, sum(accredited) as accredited FROM kdpunits where lga=$1 and type=$2`;
    try {
      // const { rows } = qr.query(getAllQ);
